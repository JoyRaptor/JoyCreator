...
public final class LayerRowRenderer {
    ...
    // derived from EditorTimelineView.COLOR_RULER_BG (Studio.PANEL) — the app's pure-neutral
    ...
    private static final int COLOR_HEADER_BG      = Studio.alpha(Studio.SUNK, 0x99); // ruler grey, semi-transparent
    private static final int COLOR_HEADER_BG_LOCK = Studio.alpha(Studio.SUNK, 0x99);
    /**
    ...
    private static final int COLOR_ROW_NAME       = Studio.INK;
    private static final int COLOR_ICON_ON        = Studio.INK;
    private static final int COLOR_ICON_OFF       = Studio.alpha(Studio.INK, 0x66);
    ...
    private static final int COLOR_SOLO_RING = Studio.CAREFUL;
    private static final int COLOR_METER_TRACK = Studio.alpha(Studio.INK, 0x26);
    private static final int COLOR_METER_FILL = Studio.GO;
    private static final int COLOR_METER_CLIP = Studio.DANGER;
     ...
     * gray (home ghost).
     */
    private static final int COLOR_DROP_TARGET_RING = Studio.ROOM_AVATAR_DEEP;
     ...
     * TEXT/STICKER items (base Studio.ROOM_AVATAR_DEEP blended 22% toward white ≈ Studio.GUIDE) — nearly
     ...
     */
    private static final int COLOR_SAME_ROW_OUTLINE = Studio.INK;
    ...
    public LayerRowRenderer(float density, @Nullable Typeface iconFont) {
        ...
        pendingLaneSlotPaint.setColor(Studio.INK_DIM);
        ...
        itemLabelPaint.setColor(Studio.INK);
        ...
        fadeKnobFillPaint.setColor(Studio.RAISED);
    ...
    private void ensureOnsetPaint() {
        if (onsetTickPaint.getColor() == 0) {
            onsetTickPaint.setColor(Studio.alpha(Studio.INK, 0x88));
    ...
    private void drawLaneAudioDrawer(@NonNull Canvas canvas, @NonNull RowLayout row,
        ...
        itemPaint.setColor(Studio.SURFACE);
    ...
    private void drawItemBody(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        if (label != null && !label.isEmpty()) {
            canvas.save();
            canvas.clipRect(x0, top, x1, bottom);
            itemLabelPaint.setColor(ghosted ? Studio.alpha(Studio.INK, 0x88) : Studio.INK);
        ...
        if (item.getSprite() != null) {
            float cy = centerY;
            float r = 4f * density;
            int diamondColor = ghosted ? Studio.alpha(Studio.INK, 0x66) : Studio.alpha(Studio.INK, 0xE6);
        ...
        if (item.getCaptionSpan() != null) {
            ...
            if (clip.hasCaptionStyleKeyframes()) {
                float cy = centerY;
                float r = 4f * density;
                spriteDiamondPaint.setColor(ghosted ? Studio.alpha(Studio.INK, 0x66) : Studio.alpha(Studio.INK, 0xE6));
                for (com.fadcam.ui.faditor.model.Clip.CaptionStyleKeyframe kf
                        : clip.getCaptionStyleKeyframes()) {
    ...
    private void drawFxBadge(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        if (fxBadgePaint == null) {
            ...
            fxBadgePaint.setColor(Studio.GUIDE);   // the app's purple accent, as the FX tool uses
    ...
    private boolean drawPassThroughBadge(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        itemSelectionPaint.setColor(Studio.INK);
    ...
    private void drawItemKeyframeDiamonds(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        kfDiamondStrokePaint.setColor(Studio.INK);
    ...
    private void drawPuppetMarks(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        for (com.fadcam.ui.faditor.puppet.PuppetTapeMarks.Mark m : marks) {
            ...
            puppetPaint.setColor(ghosted ? Studio.alpha(Studio.INK, 0x33) : (violet & 0x00FFFFFF) | Studio.alpha(Studio.GROUND, 0x80));
            for (long t : inside) {
    ...
    private void drawItemOpacityEnvelope(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        if (!kfEnvPaintsInit) {
            ...
            kfEnvLinePaint.setColor(Studio.alpha(Studio.INK, 0xCC));
            ...
            kfEnvDotPaint.setColor(Studio.alpha(Studio.INK, 0xCC));
        ...
        if (anyPreset) {
            kfEnvLinePaint.setColor(Studio.CAREFUL);
            kfEnvDotPaint.setColor(Studio.CAREFUL);
        } else {
            kfEnvLinePaint.setColor(Studio.alpha(Studio.INK, 0xCC));
            kfEnvDotPaint.setColor(Studio.alpha(Studio.INK, 0xCC));
        }
        ...
        for (int k = 0; k < ks.size(); k++) {
            ...
            if (kf.presetOwned) kfEnvDotPaint.setColor(Studio.CAREFUL);
            else kfEnvDotPaint.setColor(anyPreset ? Studio.CAREFUL : Studio.alpha(Studio.INK, 0xCC));
    ...
    private void drawSequenceTape(@NonNull Canvas canvas,
        ...
        if (sprite.isContinuesUntilBlocked()) {
            seqTickPaint.setColor(sprite.isClippedByNeighbour() ? Studio.DANGER : Studio.CAREFUL);
        ...
        seqTickPaint.setColor(Studio.alpha(Studio.INK, 0xB3));
    ...
    private void drawSequenceResizeModeHandles(@NonNull Canvas canvas, @NonNull TimedItem item,
        ...
        seqHandlePaint.setColor(absolute ? Studio.DANGER : Studio.CAREFUL);
    ...
    private void drawAudioCrossfades(@NonNull Canvas canvas, int floatingRowCount,
        ...
        for (com.fadcam.ui.faditor.model.AudioCrossfade x : audioCrossfades) {
            ...
            xfadePaint.setColor(Studio.GROUND | rgb);
            ...
            if (sel) {
                ...
                xfadePaint.setColor(Studio.INK);
                canvas.drawRoundRect(x0, seamY - halfH, x1, seamY + halfH, r, r, xfadePaint);
            }
            ...
            xfadePaint.setColor(Studio.SURFACE);
    ...
    private void drawAudioStruckTape(@NonNull Canvas canvas, @NonNull TimedItem item, float x0, float top, float x1, float bottom, @NonNull TimeToX timeToX) {
        ...
        for (long[] span : ac.getRemovedSpans()) {
            ...
            dark.setColor(Studio.alpha(Studio.INK, 0x22));
    ...
    private void drawMultiSelectionRow(@NonNull Canvas canvas, @NonNull Track t,
        ...
        for (TimedItem item : t.getItems()) {
            ...
            float x1 = Math.max(x0 + 6f * density,
                    timeToX.map(item.getTimelineStartMs() + dur));
            itemPaint.setColor(Studio.alpha(Studio.INK, 0x33));
