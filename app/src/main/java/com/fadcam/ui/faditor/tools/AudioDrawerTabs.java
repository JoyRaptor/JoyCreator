...
public final class AudioDrawerTabs {
    ...
    // Every visual below comes from ObjectDrawer.Kit, the ONE drawer kit (drawer audit
    // 2026-09-24, A1-A8). This file used to carry a private copy — its own glass alphas, a
    // drop shadow on every word, 8sp caps row labels and value pills — so Level looked unlike
    // Transform. The drawer wears the OBJECT's colour: the host calls
    // setAccent(ObjectPalette.AUDIO) before the tabs are built, and Kit reads Kit.accent().
    ...
    public static View levelTab(@NonNull Context ctx, @NonNull AudioClip clip,
        ...
        if (clip.getTrimmedDurationMs() > 0) {
            ...
            root.addView(fades);
            // The short "In" / "Out" words fit the half-width label column; each slider
            // still says its full name ("Fade in") to TalkBack.
            refreshers.add(fadeRow(ctx, fades,
                    ctx.getString(R.string.lane_b_audio_fade_in_short), true, clip, host, true));
            refreshers.add(fadeRow(ctx, fades,
                    ctx.getString(R.string.lane_b_audio_fade_out_short), false, clip, host, true));
        }
    ...
    private static Runnable levelRow(@NonNull Context ctx, @NonNull LinearLayout parent,
        ...
        bar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override public void onStopTrackingTouch(SeekBar s) {
                commitLevelGesture(ctx, clip, host, beforeKfs, beforeVol, refreshAll);
            }
        });
        ...
        // slider (same justification as PipDrawerTabs' Scale row). Underlined like the
        // Transform rows' numbers, which is what says "this number is a control".
        ...
        diamond.bind(volumeProp(ctx, clip, host, refreshAll),
    ...
    private static Runnable panRow(@NonNull Context ctx, @NonNull LinearLayout parent,
        ...
        bar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override public void onStopTrackingTouch(SeekBar s) {
                ...
                if (was == now) return;
                // Undo names are read back ("Undid …"), so they are words (A18).
                host.recordUndo(ctx.getString(R.string.lane_b_audio_pan_slider),
     ...
     */
    @NonNull
    private static ObjectMenuSheet.Prop volumeProp(@NonNull Context ctx,
                                                   @NonNull AudioParams clip, @NonNull Host host,
                                                   @Nullable Runnable refresh) {
        return new ObjectMenuSheet.Prop(
                "audioVolume", ctx.getString(R.string.lane_b_audio_level_slider), 0f, 2f,
                ...
                () -> dropKeyHere(ctx, clip, host, refresh),
                () -> nudgeToKey(clip, host, -1),
                () -> nudgeToKey(clip, host, 1),
                () -> deleteKeyHere(ctx, clip, host, refresh),
    ...
    private static Runnable envelopeStateRow(@NonNull Context ctx, @NonNull LinearLayout parent,
        ...
        TextView clear = chip(ctx, ctx.getString(R.string.faditor_kf_clear));
        ...
        clear.setOnClickListener(v -> {
            ...
            host.recordUndo(ctx.getString(R.string.lane_b_audio_undo_clear_envelope),
        ...
        return () -> {
            ...
            state.setText(n == 0
                    ? ctx.getString(R.string.lane_b_audio_env_flat)
                    : ctx.getResources().getQuantityString(
                            R.plurals.lane_b_audio_env_count, n, n));
...
private static Runnable fadeRow(@NonNull Context ctx, @NonNull LinearLayout parent,
        ...
        bar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override public void onStopTrackingTouch(SeekBar s) {
                commitFadeGesture(ctx, clip, host, fadeIn, beforeKfs, refreshAll);
            }
        });
    ...
    public static View cleanTab(@NonNull Context ctx, @NonNull AudioParams clip,
        ...
        final Runnable refresh = () -> {
            ...
            if (baking[0]) {
                state.setText(R.string.lane_b_audio_bake_processing);
                return;
            }
            if (clip.isBakedSource()) {
                state.setText(R.string.lane_b_audio_bake_in_use);
            } else {
                state.setText(R.string.lane_b_audio_bake_offline);
            }
            TextView revert = chip(ctx, ctx.getString(R.string.lane_b_audio_revert));
            hoverLabel(revert, ctx.getString(R.string.lane_b_audio_revert_desc));
            ...
            revert.setOnClickListener(v -> {
                ...
                host.recordUndo(ctx.getString(R.string.lane_b_audio_revert),
                        () -> {
        ...
        class Apply implements Runnable {
            ...
            @Override public void run() {
                ...
                cache.bakeAsync(projectDir, req,
                        frac -> main.post(() -> state.setText(frac < 0
                                ? ctx.getString(R.string.lane_b_audio_bake_processing)
                                : ctx.getString(R.string.lane_b_audio_bake_progress,
                                        label, Math.round(frac * 100f)))),
                        (result, error) -> main.post(() -> {
                            baking[0] = false;
                            BAKING_CLIP_IDS.remove(clip.getId());
                            if (result == null) {
                                state.setText(ctx.getString(R.string.lane_b_audio_bake_failed,
                                        error != null ? error : "?"));
                                return;
                            }
        ...
        TextView dn = chip(ctx, ctx.getString(R.string.lane_b_audio_denoise));
        hoverLabel(dn, ctx.getString(R.string.lane_b_audio_denoise_desc));
        dn.setOnClickListener(v -> new Apply(
                com.fadcam.ui.faditor.audio.BakedAudioCache.CHAIN_DENOISE,
                ctx.getString(R.string.lane_b_audio_denoise)).run());
        actions.addView(dn);
        TextView ln = chip(ctx, ctx.getString(R.string.lane_b_audio_normalize));
        hoverLabel(ln, ctx.getString(R.string.lane_b_audio_normalize_desc));
        ln.setOnClickListener(v -> new Apply(
                com.fadcam.ui.faditor.audio.BakedAudioCache.CHAIN_LOUDNORM,
                ctx.getString(R.string.lane_b_audio_normalize)).run());
        actions.addView(ln);

        root.addView(note(ctx, ctx.getString(R.string.lane_b_audio_bake_note)));
        return root;
    }
    ...
    public static View fxTab(@NonNull Context ctx, @NonNull Host host,
        ...
        if (clip != null) {
            ...
            android.widget.CheckBox vSwitch = checkBox(ctx, ctx.getString(R.string.lane_b_audio_voice));
            ...
            vSwitch.setOnCheckedChangeListener((b, on) -> {
                ...
                host.recordUndo(ctx.getString(R.string.lane_b_audio_voice_undo),
                        () -> clip.setVoiceFxEnabled(on),
                        () -> clip.setVoiceFxEnabled(before));
                clip.setVoiceFxEnabled(on);
                Toast.makeText(ctx, on
                                ? R.string.lane_b_audio_voice_on
                                : R.string.lane_b_audio_voice_off,
            ...
            TextView vNote = note(ctx, ctx.getString(R.string.lane_b_audio_voice_note));
        ...
        if (host.supportsAudioReactiveLink()) {
            TextView linkBtn = chip(ctx, ctx.getString(R.string.lane_b_audio_beat_link_chip));
            ...
            root.addView(note(ctx, ctx.getString(R.string.lane_b_audio_beat_link_note)));
        }

        root.addView(sectionLabel(ctx, ctx.getString(R.string.lane_b_audio_comp)));
        ...
        Runnable refresh = () -> {
            ...
            value.setText(live ? fmtGr(gr) : fmtGr(0f));
            note.setText(fxChainBypassed
                    ? R.string.lane_b_audio_comp_bypassed
                    : live ? R.string.lane_b_audio_comp_live
                    : R.string.lane_b_audio_comp_idle);
        };
        refresh.run();
    ...
    private static final class GainReductionBar extends android.view.View {
        ...
        private static final int TRACK_COLOR = ObjectDrawer.Kit.TRACK;
    ...
    // These used to be a private copy of the kit (drawer audit 2026-09-24, A1-A8). They are
    // thin now: each says which Kit piece a control is, plus the one local layout fact the
    // Kit does not know (a padding, a width).
    ...
    /**
     * Value width (dp). FIXED, not wrap: a readout that grew from "95%" to "100%" would
     * shorten the slider beside it mid-drag. The same 46dp the Transform rows use.
     */
    ...
    /**
     * Plain drawer text: body face in {@code colour}. No drop shadow — the other drawers
     * dropped theirs, and one drawer's words looking heavier was the mismatch (A2).
     */
    @NonNull
    ...
    /** Record 06 {@code .dsec}, from the Kit; the side padding lines it up with this tab's notes. */
    @NonNull
    ...
    /** A row's leading label: the Kit's 11sp row label, as every other drawer's (A2). */
    @NonNull
    private static TextView inlineLabel(@NonNull Context ctx, @NonNull String label, int widthDp) {
        return ObjectDrawer.Kit.rowLabel(ctx, label, widthDp);
    }

    /** A short explanatory line: the Kit's note, indented to this tab's text column. */
    @NonNull
    private static TextView note(@NonNull Context ctx, @Nullable String line) {
        TextView t = ObjectDrawer.Kit.note(ctx, line != null ? line : "");
        t.setPadding(dp(ctx, 8), dp(ctx, 2), dp(ctx, 8), 0);
        return t;
    }

    /** Record 06 {@code .dchip}: the Kit's chip, placed, with press feedback (A3). */
    @NonNull
    ...
    /**
     * A tap-to-type number: the Kit's value face — the SAME readout the Transform rows show
     * (A4) — underlined, as they are, to say it is editable.
     */
    @NonNull
    private static TextView valuePill(@NonNull Context ctx, int widthDp, int hoverRes) {
        TextView t = ObjectDrawer.Kit.value(ctx, widthDp);
        t.setSingleLine(true);
        t.setPaintFlags(t.getPaintFlags() | android.graphics.Paint.UNDERLINE_TEXT_FLAG);
        t.setPadding(0, dp(ctx, 6), 0, dp(ctx, 6));
    ...
    /** Record 06 {@code .dsl}, from the Kit (A5): the audio accent, because the host set it. */
    @NonNull
    ...
    /**
     * Record 06 {@code .dcb}: a platform CheckBox in the Kit's colours (A6). The whole 44dp
     * line is the target.
     */
    @NonNull
    ...
    /** Spoken name AND hover tooltip, through the Kit (A7). */
    ...
    private static void dropKeyHere(@NonNull Context ctx, @NonNull AudioParams clip,
        ...
        host.recordUndo(ctx.getString(R.string.lane_b_audio_undo_envelope_point),
    ...
    private static void deleteKeyHere(@NonNull Context ctx, @NonNull AudioParams clip,
                                      @NonNull Host host, @Nullable Runnable refresh) {
        long local = localMs(clip, host.playheadMs());
        List<VolumeKeyframe> before = copyKeyframes(clip);
        ...
        if (!removed) return;
        List<VolumeKeyframe> after = copyKeyframes(clip);
        host.recordUndo(ctx.getString(R.string.lane_b_audio_undo_delete_point),
                () -> { clip.setVolumeKeyframes(after); notifyChanged(host, refresh); },
                () -> { clip.setVolumeKeyframes(before); notifyChanged(host, refresh); });
        host.onChanged();
    }
    ...
    private static void commitLevelGesture(@NonNull Context ctx,
                                           @NonNull AudioParams clip, @NonNull Host host,
                                           @Nullable List<VolumeKeyframe> beforeKfs,
        ...
        host.recordUndo(ctx.getString(R.string.lane_b_audio_undo_level),
    ...
    private static void commitFadeGesture(@NonNull Context ctx,
                                          @NonNull AudioParams clip, @NonNull Host host,
                                          boolean fadeIn,
                                          @Nullable List<VolumeKeyframe> beforeKfs,
                                          @Nullable Runnable refresh) {
        ...
        String label = ctx.getString(fadeIn ? R.string.lane_b_audio_fade_in_slider
                : R.string.lane_b_audio_fade_out_slider);
        host.recordUndo(label,
    ...
    private static void promptForGain(@NonNull Context ctx, @NonNull AudioParams clip,
        ...
        input.setHint(R.string.lane_b_audio_prompt_level_hint);
        LinearLayout wrap = column(ctx);
        wrap.setPadding((int) pad[0], (int) pad[1], (int) pad[0], 0);
        wrap.addView(input);

        confirmDialog(ctx, ctx.getString(R.string.lane_b_audio_prompt_level), wrap, () -> {
            ...
            commitLevelGesture(ctx, clip, host, before, beforeVol, refresh);
    ...
    private static void promptForPan(@NonNull Context ctx, @NonNull AudioParams clip,
        ...
        input.setSelectAllOnFocus(true);
        input.setHint(R.string.lane_b_audio_prompt_pan_hint);
        LinearLayout wrap = column(ctx);
        wrap.setPadding((int) pad[0], (int) pad[1], (int) pad[0], 0);
        wrap.addView(input);

        confirmDialog(ctx, ctx.getString(R.string.lane_b_audio_prompt_pan), wrap, () -> {
            Float typed = leadingNumber(input.getText().toString());
            if (typed == null) return;
...
private static void promptForFadeSeconds(@NonNull Context ctx, @NonNull AudioClip clip,
        ...
        input.setSelectAllOnFocus(true);
        input.setHint(ctx.getString(R.string.lane_b_audio_prompt_fade_hint,
                trimNumber(maxFade / 1000f)));
        LinearLayout wrap = column(ctx);
        wrap.setPadding((int) pad[0], (int) pad[1], (int) pad[0], 0);
        wrap.addView(input);

        confirmDialog(ctx, ctx.getString(R.string.lane_b_audio_fade_prompt,
                ctx.getString(fadeIn ? R.string.lane_b_audio_fade_in_slider
                        : R.string.lane_b_audio_fade_out_slider)), wrap, () -> {
            Float typed = leadingNumber(input.getText().toString());
            if (typed == null) return;
            List<VolumeKeyframe> before = copyKeyframes(clip);
            long ms = Math.round(Math.max(0f, Math.min(maxFade / 1000f, typed)) * 1000f);
            setFade(clip, fadeIn, ms);
            commitFadeGesture(ctx, clip, host, fadeIn, before, refresh);
            notifyChanged(host, refresh);
        });
        input.requestFocus();
    }
