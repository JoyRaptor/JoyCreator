...
- [x] M5 schema v8 + migration + shims + dual-write + downgrade guard — LANDED 2026-07-02 compile-green (synchronized-from-flat views; ZERO call sites touched; ExportManager untouched; deserializer double-stamp bug fixed; save refused on newer-version projects). ✅ REGRESSION GATE **PASSED 2026-07-02 ~02:10 on Note 9** (fe67798 vs e46d0ac exports of the sandbox project: all 621 frames byte-identical / PSNR inf / raw video+PCM md5s identical / RMS curves identical to 6dp; only container timestamps differ). Dual-write validated in the wild (old build strips layers block on save; current build re-materializes losslessly). ⚠️ Track flags/blend/transform are serialized but rebuild-as-default (views are ephemeral) — M6/M7 must give mutated fields a persistent home (side-table keyed by track id, or promote Track to storage-of-record).
- [x] M-COMP-0 master gapless engine — LANDED 2026-07-02, FULL DEVICE ACCEPTANCE on Note 9 (probe PASSED on real fMP4: setMediaItems+ClippingConfiguration on remuxed sources; 0 frozen frames at all seams vs legacy's measured 1.79s/3.64s stalls on the same project; trim-rebuild seam freeze-free + undo byte-identical; scrub/captions/audio OK). `compositor/MasterPlaybackEngine.java` (308 lines) + FaditorPlayerManager delegation (+322) + activity resolver/seam glue (+47). Flag `FaditorPlayerManager.GAPLESS_ENGINE` line 50, DEFAULT ON (legacy path = flag off; ineligible projects — loops/transitions/images — auto-fallback to legacy). Closes user feedback #9. Known-weak: audio sync log/architecture-verified not waveform; export round-trip + bg/fg resume fix code-traced not live-tested; >100% volume boost untested in gapless.
...
- [x] M-COMP-1 image/text/sticker/sprite preview — LANDED 2026-07-02 compile-green + PARTIAL DEVICE PASS on Note 9 (new compositor/LayerPreviewController stateless query authority + LayerImageOverlayView inert surface; hidden TEXT filters at all 11 setData sites; muted AUDIO gates all 6 volume sites + live push via applyAudioTrackMuteLive; KeyframeSet.valueAt reused for transform; device-verified: launch/no-crash, mute toggle+undo+JSON round-trip, v7 stamp preserved; NOT yet device-verified: hide-tap interactive, image items (no creation UI until M10)). [x] M-EXPORT-1 — LANDED 2026-07-04 (cc09f74, device-proven): export shares LayerPreviewController authorities (z-sorted visibleTextOverlays / isAudioClipTrackMuted) = parity by shared code; GATE passed with decoded video+audio stream md5 EQUALITY before/after; hidden-track export verified; isSimpleTrim guard incl. sprites; overlay-video 2nd sequence built-inert (unreachable until video-layer creation exists; TODOs for keyframed motion/PiP XY/opacity → M-EXPORT-2). **LAYERS MVP COMPLETE** (M5→M-COMP-0→M6→M7→M-COMP-1→M10→M-EXPORT-1). FOLLOW-UPS FOUND: preview hide-toggle lacks live refresh (export correct, preview stale until reload — small fix owed); muted-track CAPTION behavior unspecified (renders today — needs user call); export-duration math oddity (25.15s actual vs 00:42 dialog — pre-existing, unowned).
- [x] M-COMP-2 live second video — LANDED 2026-07-05 (f6533f2 model/storage + 7cb75f6 creation/preview), DEVICE-VERIFIED on the Note 9 incl. save round-trip. **Part 10 probe #1 CLOSED: GO with headroom** (2 AND 3 simultaneous 1080p HEVC decoders at full ~29fps, zero steady-state drops — `compositor/DecoderBudgetProbeActivity` kept as a permanent probe tool). **Architecture note — deliberate deviation from §3.3's GL-compositor sketch:** preview uses a second ExoPlayer into a transformed `TextureView` (`compositor/OverlayVideoPreviewView`) layered above the master surfaces and below sprite/text/captions, NOT a shared GLSurfaceView — same decoder budget, no master-pipeline rewrite, NORMAL-blend preview (which is §3.3's own acceptance bar). Blend-mode preview, if ever binding, is the only thing that would force the GL compositor; probe #4's documented-approximation rule covers the gap until then. Decoder policy per plan: ONE live overlay (top-most visible), still-frame fallback for others is a delegated follow-up. [x] M-EXPORT-2 CORE — LANDED 2026-07-06, DEVICE-PROVEN (export Faditor_20260705_221335.mp4 frame-verified): PiP exports via a bottom-most overlay-video pass in `CompositeExportOverlay` (MMR frame decode per absolute timelineMs; transform sampled from the SAME KeyframeSet + the preview's DEFAULT_* constants; window end-inclusive like preview; pipFrames counter in the release summary). **Probe #3 CLOSED with a superseding finding:** `DefaultVideoCompositor` draws the PRIMARY sequence ON TOP (back-to-front loop, index 0 last) — a second video sequence composites the PiP INVISIBLY under the master, so §5.1's second-sequence sketch is unusable on stock media3 1.8; `buildOverlayVideoSequence` was DELETED. The overlay pass also gives z-parity (PiP under sprite/text/captions) that a second sequence never could. `usesLayerFeaturesAffectingExport` checks `overlayClips` explicitly. Export composites ALL simultaneous PiPs (preview live-decodes only the top-most — documented §3.3 tradeoff). REMAINING: [ ] blend modes (`BlendModeGlEffect` after the `GlTransitionExportEffect` pattern — needs the effect to sample the accumulated frame, NOT expressible via BitmapOverlay); [ ] preview overhang honesty (preview lets a PiP extend past the content rect, export clips at canvas — clamp/clip preview, delegable); [ ] MMR-per-frame perf (streaming-decoder TextureOverlay if real projects hurt); [ ] transition items don't carry overlays (pre-existing).
- [x] M10 drag-between-layers + drop-to-new-layer — LANDED 2026-07-02 compile-green (completed `ab316a5`'s WIP, which already had ~95% of the plumbing: `LayerTrackDef`, layerId-based Timeline grouping, `LayerRowRenderer` cross-row highlight + "+ New layer" zone drawn below the last row, `LayerGestureController` hover-target + same-band/locked/hidden rejection, full serialization, and the real onScroll gesture-hijack fix, all kept as-is). Fixed a genuine gap: the WIP recorded position-change and track-change as TWO undo pushes for a diagonal drag, violating "ONE undo step per drag" — merged via callback reordering + a staged-undo pattern in `FaditorEditorActivity` (`pendingLayerTrackUndo`/`mergedAction`), zero model/storage changes. Device-verified (cheap): tap-scrub + pan regression-free, no crashes, sandbox JSON ground truth confirmed clean baseline. NOT device-verified: the actual drag gesture (one scripted attempt missed the target, not iterated on per the testing rule) — hand-test checklist owed, see `handoff.md` 2026-07-02 entry.
- [ ] M11 ripple/gap toggle
