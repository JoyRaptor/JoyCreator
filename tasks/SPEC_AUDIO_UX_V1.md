...
| `A1.b` | **Sweep A1's disclosed residue.** All unreachable behind the same never-true `audioLayerTracks.isEmpty()` gate, so this is tidiness, NOT a live bug — do not treat it as urgent. (a) audio-trim drag path: `hitTestAudioHandle`, `Drag.AUDIO_LEFT_HANDLE` / `AUDIO_RIGHT_HANDLE`, `handleAudioTrimDrag`, `audioTrimDrag*` fields — its painter `drawAudioTrimHandles` is already gone, so nothing draws what it would drag. (b) `selectedAudioIndex` + its dead fallback branch in `getSelectedAudioIndex()` (the live path derives from `LayerGestureController` and must keep working). (c) orphaned listener methods `onAudioBandDoubleTapped` / `onAudioClipSelected` / `onAudioTrimChanged` / `onAudioTrimFinished` — no emitters remain. **Touches `FaditorEditorActivity`, so claim the lane in `LANES.md` first.** END STATE: zero references to any listed symbol; `typecheck.sh` green; audio select/trim on the unified rows unchanged on device | `PENDING` | `timeline/EditorTimelineView.java`, `FaditorEditorActivity.java` | |
| `A2` | Generalise `PipOverlayDrawer` → `ObjectDrawer` with a per-type tab set | `PENDING` | `tools/PipOverlayDrawer.java` (rename), `FaditorEditorActivity.java` | |
| `A3` | `MAX_HEIGHT_FRACTION` 0.55 / 0.75 branch for audio-only projects | `PENDING` | `tools/ObjectDrawer.java` | |
| `A4` | Migrate the caption drawer onto `ObjectDrawer` — kills the `0xFF1A1A1A` opacity violation | `PENDING` | `FaditorEditorActivity.buildCaptionDrawerContent` | |
...
| ID | Item | Status | Files | Evidence |
|---|---|---|---|---|
| `B1.E` | Fade in / out written as two envelope keyframes | `PENDING` | `model/AudioClip.java`, `model/VolumeEnvelope.java` | |
| `B1.U` | Fade drag handles, geometry exactly per §4 | `PENDING` | `layers/LayerRowRenderer.java`, `layers/LayerGestureController.java` | |
...
| `B8` | Rename carousel "Audio" → "Extract audio", re-icon `call_split` | `PENDING` | `tools/FaditorToolRegistry.java` | |
...
| `B10` | **Give audio clips the range chips.** **JOYRAPTOR RULED 2026-08-22: ADOPT — build it alongside `B1.U`, do not sequence it after.** `showObjectMenuSheetForAudioClip` passes `rangeChips = null` (`FEA:21521`), so audio is the only object without `⇤ Start here` / `End here ⇥`. **TWO chips, not three** — `↔ Span whole` is meaningless for audio (a clip's length IS its sound; it cannot stretch without looping or time-stretch). Mechanical: pass the two suppliers, reuse `ic_marker_flag_start` / `ic_marker_flag_end`. Reverses the *"audio trim lives on the band"* note above the call site — that note must be UPDATED, not silently contradicted | `PENDING` | `FaditorEditorActivity.java:21521` | |
