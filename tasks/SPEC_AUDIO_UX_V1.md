...
| `C2.E` | Baked chain: `afftdn` denoise, two-pass `loudnorm`, revertible cache | `WIP` — **BUILT was FALSE: the file did not compile.** Committed at `ebc9144e` with a report claiming "TYPECHECK OK — 634 sources"; the actual harness gave **8 errors**. `result()` was declared `String` while returning a `Result` (5), and `FFmpegKit.execute(cmd, logCb, statsCb)` **has no such overload** — the working call in this app is the 1-arg synchronous form at `TranscriptionEngine:517` (3). Compile repaired by review at `7f396ce2`; per-frame progress dropped on the sync path, which had no UI to drive. **The ffmpeg BEHAVIOUR is still entirely unverified — it has never been executed once.** Row stays `WIP` until afftdn and two-pass loudnorm are proven to actually produce a processed file on device | new `faditor/audio/BakedAudioCache.java` | TYPECHECK OK 634 sources (real ffmpeg-kit jars on the classpath — FFmpegKit/FFmpegKitConfig/Statistics signatures compile). NOT RUN: ffmpeg never executed (no device) — **the §6.2 assumption that ffmpeg-kit ships afftdn+loudnorm and pass-1 JSON lands in session LOGS is UNTESTED and flagged in the class javadoc; if wrong it surfaces as logged rc + log tail, never silent success.** Storage constraint honored: takes caller-supplied projectDir (`ProjectStorage.projectDir(id)` at wiring), refuses getCacheDir. ExportManager NOT touched; Clean tab NOT built (C2.U owns it) |
