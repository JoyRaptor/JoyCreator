...
| `A6` | Project sample-rate policy: one rate, everything resampled in | `PENDING` — **harness green, rate-selection unified (`G20`), device export through the UNIFIED path still owed** (see §0 rule 9) | `export/ExportManager.java`, `export/ResamplingAudioProcessor.java`, `tools/jvm-harness/ResampleTest.java`, `tools/jvm-harness/run-resample.sh`, `tasks/export_resample_probe.py` | `bash tools/jvm-harness/run-resample.sh` → `ALL GREEN` (duration 12000 exact, pitch 1000.0 Hz, level 0.998, chunked 12001 vs mono 12000, chunked pitch 999.9 Hz, chunked-vs-mono rel RMS 0.00001 <0.02 seam bound, **NEGCTRL** 1:1 copy caught by pitch @1085 Hz, 10% loss caught by duration, one-sample phase slip 0.13058 caught by 0.02 bound — teeth proven). Resampler root-caused and rewritten in bb095d21: the old accumulator carried the wrong fractional phase across queueInput calls (997-frame chunks measured 13044 frames vs 12000 monolithic) and every chunk boundary injected zero-fade artifacts. Device proof owed: real 44100-source export through `tasks/export_resample_probe.py` (broken chipmunk artifact captured in `tasks/a6/`). **A real export WAS measured 2026-08-23** (`tasks/a6/export_latest.m4a`, pulled from `/storage/emulated/0/Android/data/com.fadcam.beta/files/FadCam/Faditor/`): pitch 1000.0 Hz exact, duration 6.016s, 6/6 bursts, spacing drift 0.022s — `ALL GREEN`. **But that export predates `G20`, and logcat proves OUR resampler never ran** (no `A6: ... resampled` line; the only conversion was `AudioMixer: create resampler 44100 -> 48000` from PID 1114, Android's own audio system). So the measurement shows the PLATFORM resamples correctly, not that A6 does — A6's value is deterministic control, not rescuing audible breakage. Re-run the probe on an export from a post-`G20` build to close this row. Probe fix while measuring: drift is now measured as burst SPACING relative to the first onset, because a real export carries a constant ~41ms encoder lead-in that is not warp (it was scoring a correct export at 0.062s "drift" vs a true 0.022s) |
| `A7` | Audio parameters carried on a shared type so a *video clip's* audio gets the identical drawer (§2.2) | `BUILT` bd21c9f6 | `model/AudioParams.java`, `model/AudioClip.java`, `model/Clip.java`, `model/VolumeKeyframe.java` | `AudioParams` interface + shared `VolumeKeyframe` base; both `Clip` and `AudioClip` implement it. The wiring lived only on `joy-creator` and the fixes only on the detached line — reconciled in bd21c9f6. **Shipped with a field-shadowing bug in BOTH subclasses that zeroed every keyframed clip; see `G19`.** TYPECHECK OK — 649 sources, 1796 classes; `run-envelope.sh` ALL GREEN + ALL PASS. |
...
| ID | Item | Status | Files | Evidence |
|---|---|---|---|---|
| `C1.E` | Real-time FX chain: EQ · compressor · limiter · gate · de-esser · de-hum, shared preview/export | `PENDING` — **harness green + three behavior bugs fixed, preview/export parity on device still owed** (see §0 rule 9) | `faditor/audio/fx/EqProcessor.java`, `faditor/audio/fx/CompressorProcessor.java`, `faditor/audio/fx/GateProcessor.java`, `faditor/audio/fx/LimiterProcessor.java`, `faditor/audio/fx/DeEsserProcessor.java`, `faditor/audio/fx/DeHumProcessor.java`, `faditor/audio/fx/FxChain.java`, `waveform/Biquad.java`, `tools/jvm-harness/AudioFxTest.java`, `tools/jvm-harness/run-audio-fx.sh` | `bash tools/jvm-harness/run-audio-fx.sh` → `ALL GREEN (19/19)` (Eq +6dB@3kHz ×1.995, 200 Hz ×1.003 / 12 kHz ×1.031 untouched; Compressor quiet ×1.000, loud ×0.286, differential; Gate closed ×0.036 near-silence absolute, open ×1.000, re-closes after passage; empty FxChain bit-exact passthrough; voice chain end-to-end floor ×0.101 voice ×0.319; **NEGCTRL** passthrough EQ, constant-gain compressor, always-open gate — all caught). Harness drove THREE real fixes: (1) Eq was attenuating bandpass ignoring gainDb → `Biquad.peaking` (bb095d21); (2) ALL six processors limited their output buffer's ShortBuffer VIEW instead of the parent ByteBuffer — views have independent limit, so getOutput() shipped capacity-sized stale samples whenever a smaller queueInput reused a larger buffer (274e96ea); (3) FxChain drained its last child twice — getOutput() swaps in EMPTY_BUFFER, so every non-empty chain emitted SILENCE — plus unflipped copy (274e96ea). Row stays `PENDING` until preview/export parity proven on device |
...
| `C5.E` | Ducking: sidechain envelope generator writing **real, editable** keyframes | `BUILT` 669e30b9 | `faditor/audio/Ducker.java`, `faditor/audio/DuckApplier.java`, `tools/jvm-harness/DuckerTest.java`, `tools/jvm-harness/run-duck.sh` | `Ducker.compute(envelope, fps, keyStartMs, targetStartMs, targetDurationMs, params)` → sparse multiplier keyframes (B1.Q). **At most FOUR keyframes per speech region** — sparseness is a correctness property here, since a keyframe per analysis frame satisfies every gain assertion and is still unusable by hand, so the harness asserts the COUNT. Gaps under `bridgeGapMs` (700ms default) merge, which is what separates ducking from pumping. `DuckApplier.apply(clip, curve)` returns an unexecuted `EditAction` plus `replacedKeyframes` so the caller can warn before displacing a hand-drawn envelope (rule 7). **Harness-driven fix:** the threshold was a fraction of the key track's PEAK, so constant room tone had every frame above 15% of its own peak and ducked the music from beginning to end — caught by the silent-key negative control; now measured from the track's FLOOR (10th percentile) with a `MIN_CONTRAST` gate. `bash tools/jvm-harness/run-duck.sh` → `ALL PASS` (37 checks incl. no-pumping across a 300ms gap, recovery across a long one, offset targets, a clip starting mid-sentence starting already ducked, undo restoring hand-drawn values exactly, redo after undo, and an un-aliasing control; **NEGCTRL** silent key / duck-by-nothing / threshold above peak / deeper-really-is-deeper). `C5.U` (the way in) still owed — deferred because it shares the track-header menu with `B3`, held by another lane |
...
| `C6` | Compressor gain-reduction bar (a compressor tuned blind is guesswork) | `BUILT` | `tools/AudioDrawerTabs.java` | New FX tab (`AudioDrawerTabs.fxTab`) on BOTH audio drawers (standalone + §2.2 clip-audio shelf, identical per contract): "Compressor" card with a horizontal GR meter (fill grows RIGHT-to-LEFT — reduction reads as subtraction) scaled 0..−12 dB, dB readout beside it. Bar ALWAYS drawn at 0.0 dB resting state (G18). Engine hook shipped first so wiring has a consumer day one: `AudioDrawerTabs.reportGainReductionDb(float)` writes a volatile the tab reads on every playhead-tick refresh (same `faditor_tag_row_refresh` channel as Level). §0-rule-6 honesty: standing note states exactly whether the value is live or still waiting for C1.E wiring. NOTE: spec §2.1 lists compressor under LEVEL; built as FX tab per JoyRaptor's row assignment — one home must win when Tone/FX fill in |
| `C7` | A/B bypass — one tap mutes the whole FX chain for comparison | `BUILT` | `tools/ObjectDrawer.java` (via its generic Toggle API — zero chrome changes needed), `tools/AudioDrawerTabs.java`, `FaditorEditorActivity.java` | Header toggle (ic_fx_24, danger-tinted when engaged) beside mute/lock on BOTH audio drawers — reachable from EVERY tab, which is the point of A/B while tuning. State = session-scoped volatile `AudioDrawerTabs.fxChainBypassed`; toast confirms "A/B: FX chain OFF/ON". §0-rule-6 honesty: no engine reads the flag until C1.E preview/export wiring lands, so the FX tab carries the standing note saying exactly that; the switch ships first so the engine wires to an existing consumer. NOT device-heard (nothing to hear yet — that is the disclosed point) |
...
| `D11` | **Transcript-keyed ducking option for `C5.E`:** use Whisper word timings as the gate signal instead of (or alongside) amplitude sidechain — no pumping artifacts, deterministic, and it makes ducking *editable text*. Generator still writes ordinary keyframes per §3.5 | `BUILT` (see tail) | `faditor/audio/Ducker.java`, `tools/jvm-harness/DuckerTest.java`, `tools/jvm-harness/run-duck.sh` | `Ducker.computeFromWords(words, keyStartMs, keyInPointMs, targetStartMs, targetDurationMs, params)` maps each unstruck word from the key clip's SOURCE timeline onto the timeline and reuses `C5.E`'s shared `fromRegions` stage, so the two signals cannot drift apart — only "what counts as speech" differs. **Struck words duck nothing**: a struck word is deleted from the render, is never heard, and ducking under it is the precise failure this signal exists to avoid. `bash tools/jvm-harness/run-duck.sh` → `ALL PASS` (37 checks). D11 half: ducked on the first word, stays down in a 100ms inter-word gap, recovered after the last, ≤4 keyframes for two adjacent words, trim in-point + clip offset both applied; **NEGCTRL** struck word ducks nothing / the same word unstruck DOES duck (so the strike check is not vacuous) / no transcript means no keyframes / a zero-length recogniser artefact is discarded |
