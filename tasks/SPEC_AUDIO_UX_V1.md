...
| `B6` | Audio clip **double-tap** opens the four-tab drawer (today: nothing) | `PENDING` | `FaditorEditorActivity.java:14115` | **WIRING CONTRACT, delivered by LANE C 2026-08-22 (`d6ef6106`):** call `AudioDrawerTabs.levelTab(Context, AudioClip, AudioDrawerTabs.Host)`. `Host` is `{ long playheadMs(); void seekTo(long timelineMs); void onChanged(); void recordUndo(String, Runnable, Runnable); }` — note `seekTo` is one method BEYOND `PipDrawerTabs.Host`, needed for keyframe-diamond ‹ › navigation, so the activity must supply it. Level is the ONLY tab that exists; Clean/Tone/FX arrive with `C1.E`. |
