...
| `A6` | Project sample-rate policy: one rate, everything resampled in | `PENDING` — **harness written, not yet proven BUILT** (see §0 rule 9) | `export/ExportManager.java`, `export/ResamplingAudioProcessor.java`, `tools/jvm-harness/ResampleTest.java`, `tools/jvm-harness/run-resample.sh` | `bash tools/jvm-harness/run-resample.sh` → `ALL GREEN` (duration 12000/12000, pitch 1000.0 Hz ±25, level 0.998, chunked 12001 vs 12000, chunked pitch 999.9, chunked vs monolithic 0.145 <0.20, **NEGCTRL** 1:1 copy caught by pitch 1085 Hz, 10% loss caught by duration) — harness has teeth, but row stays `PENDING` until device export proves it |
...
| ID | Item | Status | Files | Evidence |
|---|---|---|---|---|
| `C1.E` | Real-time FX chain: EQ · compressor · limiter · gate · de-esser · de-hum, shared preview/export | `PENDING` — **harness written, not yet proven BUILT** (see §0 rule 9; Eq was attenuating, fixed via `Biquad.peaking`) | `faditor/audio/fx/EqProcessor.java`, `faditor/audio/fx/CompressorProcessor.java`, `faditor/audio/fx/GateProcessor.java`, `waveform/Biquad.java`, `tools/jvm-harness/AudioFxTest.java`, `tools/jvm-harness/run-audio-fx.sh` | `bash tools/jvm-harness/run-audio-fx.sh` → `ALL PASS` (Eq boost 2.503 >1.2, other 1.18 in 0.7-1.35, Compressor loud 0.300 <0.95 quiet 1.0, Gate quiet 0.0007 <0.01 loud 1.0, **NEGCTRL** Eq 0dB 1.0, Compressor ratio1 1.0, Gate always-open 0.0022 — all teeth) — harness has teeth, but row stays `PENDING` until preview/export parity proven on device |
