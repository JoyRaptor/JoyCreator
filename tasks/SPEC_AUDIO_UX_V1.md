...
| `A6` | Project sample-rate policy: one rate, everything resampled in | `BUILT` 71994dc6 | `export/ExportManager.java`, `export/ResamplingAudioProcessor.java`, `model/AudioClip.java` | Project rate picked from master track's first audio source (default 48 kHz). New `ResamplingAudioProcessor` (linear interpolation) inserted at chain input for any clip whose source rate ≠ project rate — applied in master track, audio lanes, and PiP audio paths. Probe-verified: 44.1 kHz source in 48 kHz project renders at correct pitch/duration (no chipmunk, no desync). TYPECHECK OK — 639 sources, 1785 classes (pre-existing Clip.java issues excluded). |
| `A7` | Audio parameters carried on a shared type so a *video clip's* audio gets the identical drawer (§2.2) | `BUILT` 3372d938 | `model/AudioClip.java`, `model/Clip.java`, `model/AudioParams.java`, `model/VolumeKeyframe.java`, `tools/AudioDrawerTabs.java` | Created `AudioParams` interface with all audio parameters/operations; `Clip` and `AudioClip` now implement it; shared `VolumeKeyframe` class; `AudioDrawerTabs` uses `AudioParams` instead of `AudioClip`. `showAudioDrawer`/`showClipAudioDrawer` now pass the object directly — no synthetic `AudioClip` copy. `TYPECHECK OK — 639 sources, 1784 classes`. |
...
| ID | Item | Status | Files | Evidence |
|---|---|---|---|---|
| `C1.E` | Real-time FX chain: EQ · compressor · limiter · gate · de-esser · de-hum, shared preview/export | `BUILT` 6c33883a | new `faditor/audio/fx/`, `export/ExportManager.java` | New `faditor/audio/fx/` engine: `EqProcessor` (peaking via `Biquad.bandChain`), `CompressorProcessor`, `LimiterProcessor` (lookahead), `GateProcessor`, `DeEsserProcessor` (FFT spectral gate), `DeHumProcessor` (Biquad notch at 50/60 Hz + harmonics), `FxChain` composes them in series. All processors are `BaseAudioProcessor` subclasses shared by preview and export. Preset chains: `createFixChain` (HP80→DeEsser→Compressor→Limiter) and `createVoiceChain`. Tone/FX tab UI deferred per §0 rule 6. TYPECHECK OK — 639 sources, 1785 classes (pre-existing Clip.java issues excluded). |
