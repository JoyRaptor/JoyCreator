...
| `B2.U` | Pill render + gestures per §5, mirroring the transition pattern | `WIP` — **part 1 of 2 built** (`d6228d4e`): the pill draws at the lane seam with §5.1's corrected shading (loser darkens after, winner darkens before), direction arrows, assignable colour, and a selected state. **PART 2 IS NOT DONE: no gestures and no creation path, so nothing can make a pill yet and it is untestable by hand.** Remaining work, all in files this lane already holds: (a) `hitTestCrossfade` at 24dp inflated per §5.2, losing to trim and fade handles; (b) middle-drag slides via `moveTo`, edge-drag resizes via `setEdge` — mirror the shipped transition pattern (`hitTestTransition` / `hitTestTransitionHandle` / `drawTransitionTrimHandles`) rather than inventing a second one; (c) the fluent creation path of §5.4 — drag clip B's fade-in handle left past clip A's end on an adjacent lane and it snaps into a pill. The discoverable `+ Cross-fade` door is `B2.U2` and lives in LANE C's `AudioDrawerTabs` | `timeline/EditorTimelineView.java`, `layers/LayerRowRenderer.java` | |
