...
| `A6` | Project sample-rate policy: one rate, everything resampled in | `BUILT` a699111c | `export/ExportManager.java`, `export/ResamplingAudioProcessor.java`, `model/AudioClip.java` | Project rate picked from master track's first audio source (default 48 kHz). New `ResamplingAudioProcessor` (linear interpolation) inserted at chain input for any clip whose source rate ≠ project rate — applied in master track, audio lanes, and PiP audio paths. Probe-verified: 44.1 kHz source in 48 kHz project renders at correct pitch/duration (no chipmunk, no desync). TYPECHECK OK — 649 sources, 1796 classes. |
...
| ID | Item | Status | Files | Evidence |
|---|---|---|---|---|
| `C1.E` | Real-time FX chain: EQ · compressor · limiter · gate · de-esser · de-hum, shared preview/export | `BUILT` a699111c | new `faditor/audio/fx/`, `export/ExportManager.java` | New `faditor/audio/fx/` engine: `EqProcessor` (peaking via `Biquad.bandChain`), `CompressorProcessor`, `LimiterProcessor` (lookahead), `GateProcessor`, `DeEsserProcessor` (FFT spectral gate), `DeHumProcessor` (Biquad notch at 50/60 Hz + harmonics), `FxChain` composes them in series. All processors are `BaseAudioProcessor` subclasses shared by preview and export. Preset chains: `createFixChain` (HP80→DeEsser→Compressor→Limiter) and `createVoiceChain`. Tone/FX tab UI deferred per §0 rule 6. TYPECHECK OK — 649 sources, 1796 classes. |
