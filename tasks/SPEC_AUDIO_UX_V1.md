...
| `B1.Q` | **A fade silently caps a boosted clip at 100%. Real user-visible bug, found by LANE C 2026-08-22.** `armFade` and `setFadeInMs` write unity `1f` as the fade's full value, and `VolumeEnvelope.gainAt` returns the flat `volumeLevel` **only when there are no keyframes at all** — once any exist the envelope wins outright. So: set a clip to 150%, then drag a fade, and it silently drops to 100% for its whole length. **Decide the model rule, then apply it in ONE place:** either the envelope stores absolute gain and a fade's full value is the clip's current `volumeLevel` (not `1f`), or the envelope becomes a 0..1 MULTIPLIER over `volumeLevel` — cleaner, but it changes the meaning of every stored keyframe and needs a project migration. **JoyRaptor's call; do not pick unprompted.** Affects `armFade`, `setFadeInMs`/`setFadeOutMs`, `gainAt`, and the export processor equally — preview and file must not diverge | `PENDING` — awaiting JoyRaptor's ruling | `model/VolumeEnvelope.java`, `model/AudioClip.java`, `layers/LayerGestureController.java`, `export/VolumeAudioProcessor.java` | |
...
| `C2.E` | Baked chain: `afftdn` denoise, two-pass `loudnorm`, revertible cache | `BUILT` ebc9144e — ENGINE ONLY, no UI (C2.U is next). New `audio/BakedAudioCache`: Request(sourceUri, startMs, endMs, chain) → Result(bakedFile + originalSourceUri + key + chain); chains `CHAIN_DENOISE` (afftdn nf=-25dB) and `CHAIN_LOUDNORM` (two-pass EBU R128, I=-16/TP=-1.5/LRA=11, pass-2 linear=true, measured_* parsed from pass-1 logs by regex, `-inf` round-trips as string); bakeAsync (progress 0..1 across whole bake, -1 indeterminate) + blocking bakeSync; cache key = sourceUri+range+chain+KEY_VERSION under `<projectDir>/audio_baked/`; temp-then-rename commit; <1024B artifacts discarded not served; discardBake deletes ONLY the bake — original structurally never opened for write | new `faditor/audio/BakedAudioCache.java` | TYPECHECK OK 634 sources (real ffmpeg-kit jars on the classpath — FFmpegKit/FFmpegKitConfig/Statistics signatures compile). NOT RUN: ffmpeg never executed (no device) — **the §6.2 assumption that ffmpeg-kit ships afftdn+loudnorm and pass-1 JSON lands in session LOGS is UNTESTED and flagged in the class javadoc; if wrong it surfaces as logged rc + log tail, never silent success.** Storage constraint honored: takes caller-supplied projectDir (`ProjectStorage.projectDir(id)` at wiring), refuses getCacheDir. ExportManager NOT touched; Clean tab NOT built (C2.U owns it) |
