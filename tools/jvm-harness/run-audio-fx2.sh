#!/usr/bin/env bash
# AudioFx2Test — C1.E Eq / Compressor / Gate, off device.
#
...
OUT=tools/jvm-harness/out-audio-fx2
...
javac @"$ARGS" tools/jvm-harness/AudioFx2Test.java || exit 1
[ -f "$OUT/AudioFx2Test.class" ] || { echo "no AudioFx2Test class — compile did not run"; exit 1; }
java @"$RUNARGS" AudioFx2Test || exit 1
