#!/usr/bin/env bash
# BeatDetectorTest + BeatDetectorTest — D6 beat + onset detection, off device.
#
...
#
# Usage:  bash tools/jvm-harness/run-beats.sh
set -u
cd "$(dirname "$0")/../.." || exit 1
...
OUT=tools/jvm-harness/out-beats
...
javac @"$ARGS" tools/jvm-harness/BeatDetectorTest.java || exit 1
# Positive control on the COMPILE itself — the run-matte lesson.
[ -f "$OUT/BeatDetectorTest.class" ] || { echo "no BeatDetectorTest class — compile did not run"; exit 1; }
java @"$RUNARGS" BeatDetectorTest || exit 1
