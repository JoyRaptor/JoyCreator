#!/usr/bin/env bash
# DuckerTest — C5.E sidechain ducking, off device.
#
...
OUT=tools/jvm-harness/out-duck
...
javac @"$ARGS" tools/jvm-harness/DuckerTest.java || exit 1
# Positive control on the COMPILE itself — the run-matte lesson.
[ -f "$OUT/DuckerTest.class" ] || { echo "no DuckerTest class — compile did not run"; exit 1; }
java @"$RUNARGS" DuckerTest || exit 1
