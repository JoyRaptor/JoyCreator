#!/usr/bin/env bash
# LaneChainTest — C1.E Eq / Compressor / Gate, off device.
#
...
OUT=tools/jvm-harness/out-lanechain
...
javac @"$ARGS" tools/jvm-harness/LaneChainTest.java || exit 1
[ -f "$OUT/LaneChainTest.class" ] || { echo "no LaneChainTest class — compile did not run"; exit 1; }
java @"$RUNARGS" LaneChainTest || exit 1
