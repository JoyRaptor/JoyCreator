#!/usr/bin/env bash
# Mesh deformation ENGINE (SPEC_20260902_MESH_WARP + SPEC_20260904_PUPPET_ARCHITECTURE).
...
OUT=tools/jvm-harness/out-mesh
...
javac @"$ARGS" tools/jvm-harness/MeshEngineTest.java || exit 1
...
[ -f "$OUT/MeshEngineTest.class" ] || { echo "no MeshEngineTest class — the compile did not run"; exit 1; }
