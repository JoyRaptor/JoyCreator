#!/usr/bin/env bash
# ParagraphTest — C5.E sidechain ducking, off device.
#
...
OUT=tools/jvm-harness/out-para
...
javac @"$ARGS" tools/jvm-harness/ParagraphTest.java || exit 1
# Positive control on the COMPILE itself — the run-matte lesson.
[ -f "$OUT/ParagraphTest.class" ] || { echo "no ParagraphTest class — compile did not run"; exit 1; }
java @"$RUNARGS" ParagraphTest || exit 1
