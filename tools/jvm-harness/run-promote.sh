#!/usr/bin/env bash
# PromoteDemoteTest — §3a track-matte visibility, off device, seconds to run.
#
...
OUT=tools/jvm-harness/out-promote
...
javac @"$ARGS" tools/jvm-harness/PromoteDemoteTest.java || exit 1
# The positive control on the COMPILE itself: an empty out dir means the command never ran.
[ -f "$OUT/PromoteDemoteTest.class" ] || { echo "no class file — the compile did not run"; exit 1; }
java @"$RUNARGS" PromoteDemoteTest
