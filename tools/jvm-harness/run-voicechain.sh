#!/usr/bin/env bash
# VoiceChainTest — C1.E Eq / Compressor / Gate, off device.
#
...
OUT=tools/jvm-harness/out-voicechain
...
javac @"$ARGS" tools/jvm-harness/VoiceChainTest.java || exit 1
[ -f "$OUT/VoiceChainTest.class" ] || { echo "no VoiceChainTest class — compile did not run"; exit 1; }
java @"$RUNARGS" VoiceChainTest || exit 1
