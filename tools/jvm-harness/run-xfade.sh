#!/usr/bin/env bash
# CrossfadeTest — C5.E sidechain ducking, off device.
#
...
OUT=tools/jvm-harness/out-xfade
...
javac @"$ARGS" tools/jvm-harness/CrossfadeTest.java || exit 1
# Positive control on the COMPILE itself — the run-matte lesson.
[ -f "$OUT/CrossfadeTest.class" ] || { echo "no CrossfadeTest class — compile did not run"; exit 1; }
java @"$RUNARGS" CrossfadeTest || exit 1
